...
- `ChatGPTConversationHostMonitor` owns the stable assistant-identity/nonempty-body/non-streaming readiness check and one page-level debounce. An official action row is not required for capture. User prompt lookup is best-effort; missing prompt and assistant-only mounting are admissible. An explicit `?message=` route does not ask this monitor to materialize every persistent slot.
